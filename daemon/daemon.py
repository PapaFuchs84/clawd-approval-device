...
# The firmware does a full screen redraw on every state update (fillScreen +
# text + sprite compositing + pushSprite). Under a burst of rapid state
# broadcasts (several tool calls in quick succession) a too-tight timeout
# here can make the daemon wrongly treat an otherwise healthy device as
# unreachable and drop it, forcing a reconnect cycle. Values below are
# confirmed stable on real hardware under normal Claude Code hook traffic.
ACK_TIMEOUT_S = 4
MAX_RETRIES = 3
